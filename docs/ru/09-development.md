...
требует, чтобы каждый артефакт был непустым обычным файлом. Хелпер создаёт
каталог отчётов с режимом `0755` и нормализует готовые артефакты до режима
`0644`; ошибка scanner или проверки артефакта завершает шаг. Существующие
