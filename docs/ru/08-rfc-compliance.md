...
чья micro-BFD-сессия перешла в Down, из load-balancing table LAG. GoBFD сейчас
обнаруживает и публикует per-member и aggregate state, но ещё не управляет
Linux bonding, team или OVS membership. Для полного RFC 7130 enforcement в
Linux нужен будущий netlink/sysfs/OVS actuator или внешний operator, который
читает состояние GoBFD и применяет dataplane-изменение.
