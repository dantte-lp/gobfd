...
| Linux Micro-BFD enforcement | Partial | GoBFD creates per-member RFC 7130 sessions, tracks aggregate state, has dry-run actuator wiring, and supports explicit Linux kernel-bond sysfs enforcement with `owner_policy: allow-external`; OVS/NetworkManager membership changes need future backends. |
...
| **S7** | Independent production integration assets. | In progress: generic runbooks, Kubernetes manifest hardening, Prometheus rule correction, FRR/GoBGP example documentation, public EOS verification notes, Micro-BFD actuator policy/config wiring, and kernel-bond backend are done; remaining work is S7.1d/S7.2. Commit: `feat(examples): add production integration assets`. |
...
| **S7.1** | Linux Micro-BFD actuator. | In progress: `MicroBFDActuator` hook, guarded `netio.LAGActuator` policy, config validation, daemon dry-run wiring, and explicit kernel-bond backend are done; OVS/NetworkManager backend remains. Commit: `feat(netio): add linux lag actuator`. |
...
| **S7.1d** | Owner-aware OVS/NetworkManager backends. | Add OVS command backend and optional NetworkManager D-Bus backend without making NetworkManager a required dependency. Commit: `feat(netio): add owner-aware lag backends`. |
...
| R6 | Micro-BFD is mistaken for universal Linux LAG enforcement. | M | H | Keep docs explicit: current code enforces only explicit kernel-bond sysfs with operator-approved external ownership; OVS and NetworkManager ownership need S7.1d backends. |
