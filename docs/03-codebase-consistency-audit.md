...
the RFC docs describe, but `AddSession` and `gobfdctl session add` currently
expose only single-hop and multi-hop sessions. The primary functional gap for
`um-docs` production use is therefore not the BFD packet engine; it is the
operator-facing surface: API/CLI coverage, Kubernetes packaging, Arista/FRR
examples, and failure-drill documentation.
...
VXLAN, and Geneve through declarative config and daemon reconciliation. The
protobuf API and `gobfdctl session add` still model only:

- `SESSION_TYPE_SINGLE_HOP`
- `SESSION_TYPE_MULTI_HOP`

Impact: automation can manage advanced sessions only by editing YAML and
triggering reload. For `um-docs` style operations, this is insufficient for
dynamic failover drills, GitOps controllers, or incident tooling.

Next sprint: S5, `feat(api): expose advanced bfd session types`.
