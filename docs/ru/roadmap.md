...
режима в Up, используя существующие передачи. H-05 остаётся частичной:
очередь изменений, четырёхсторонняя матрица таймеров и интеграция обновлений
остаются в `.1.1.1`. Задача `gobfd-qj0.8.2.1.1.2.1` добавляет пересчёт TX deadline по параметрам
