...
попытками. Артефакты v0.6.4 проверены, а release tracking открыт повторно для
исправления опубликованных notes и синхронизации стабильных changelog в
`master`. Три ранее найденных P1 отдельно отслеживаются в Beads. Защищённая
линия `release/v0.6` сохраняет GoBGP v3.37.0; разработка v1 и переход на GoBGP
v4 продолжаются в `dev`.
...
остаётся **в работе** до завершения исправления release notes/default branch и
независимой проверки P1 `gobfd-qj0.8.1.8.8`, `.8.9` и `.8.10`.
...
Release-задача `gobfd-qj0.8.1.15` снова **в работе**: неизменяемый `v0.6.4`
указывает на `b1c0bcd7d2e9abed00368b2082e34f521084c087`, все 12 assets и OCI
indexes остаются проверенными, но cumulative release notes и стабильные
changelog в `master` требуют исправления. Qualification `gobfd-qj0.8.1.7` и
независимая проверка `gobfd-qj0.8.1.8` остаются открыты на трёх P1 выше.
