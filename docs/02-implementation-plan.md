...
| Linux Micro-BFD enforcement | Partial | GoBFD creates per-member RFC 7130 sessions, tracks aggregate state, and has dry-run actuator wiring; Linux bond/team/OVS/NetworkManager membership changes need a future backend. |
...
| **S7** | Independent production integration assets. | In progress: generic runbooks, Kubernetes manifest hardening, Prometheus rule correction, FRR/GoBGP example documentation, public EOS verification notes, and Micro-BFD actuator policy/config wiring are done; remaining work is S7.1c/S7.2. Commit: `feat(examples): add production integration assets`. |
...
| **S7.1** | Linux Micro-BFD actuator. | In progress: `MicroBFDActuator` hook, guarded `netio.LAGActuator` policy, config validation, and daemon dry-run wiring are done; Linux bond/team/OVS/NetworkManager backend remains. Commit: `feat(netio): add linux lag actuator`. |
...
| **S7.1c** | Linux LAG backend implementation. | Add owner-aware kernel bond, OVS, and optional NetworkManager D-Bus backend implementation for enforce mode. Commit: `feat(netio): add linux lag actuator backend`. |
...
| R6 | Micro-BFD is mistaken for full Linux LAG enforcement. | M | H | Keep docs explicit: current code has a hook, policy gate, and dry-run config wiring, but full Linux enforcement needs S7.1c backend implementation. |
