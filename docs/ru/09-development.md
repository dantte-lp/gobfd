...
соответствующих ссылок Git. Стабильный тег указывает на точный проверенный
коммит в подходящей релизной ветке; такой тег никогда не перемещают, не удаляют
и не используют повторно. GitHub Actions по событию тега создаёт черновик
релиза GitHub, полностью формирует и проверяет его описание и артефакты и только
затем публикует релиз как последнее изменение.
