...
После приёмки и публикации исправления принятая стабильная история и оба
changelog должны попасть в `master` до завершения release closeout.
Сопровождающие отдельно оценивают применимый forward-port в `dev`. Подготовка
релиза ведётся в соответствующей поддерживаемой линии.
...
5. **GitHub Actions** публикует релиз через черновик:
   ...
   - Извлекает из CHANGELOG.md все maintenance-секции от версии 0.6.2 до
     предыдущего опубликованного стабильного релиза в той же линии, не включая
     его секцию. Первая версия в новой линии использует наибольший предыдущий
     опубликованный стабильный SemVer-тег среди всех линий.
   - Добавляет ссылки на changelog и сравнение, привязанные к неизменяемому
     тегу; ссылка на изменяемый `master` не является release notes.
...
6. **Завершайте работу с релизными ветками** только после независимой проверки
   опубликованного тега, артефактов, notes и OCI evidence. Доставьте принятую
   стабильную историю и оба changelog в `master`, перенесите применимые
   изменения в `dev` и проверьте все три удалённые вершины веток до закрытия
   release tracking.
