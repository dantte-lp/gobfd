...
| Vendor NOS lab | `make interop-clab` | Optional containerlab profile for primary Arista cEOS, Nokia SR Linux, SONiC-VS, VyOS, baseline FRR, and deferred Cisco XRd. |
...
| Acceptance | `make e2e-linux` writes `go-test.json`, `go-test.log`, `containers.json`, `containers.log`, `environment.json`, `summary.md`, `link-events.json`, and `lag-backends.json`; no host interface is modified; destructive operations are constrained to a disposable `--network none` Podman namespace or fake sysfs tree. |
| Status | Implemented. |
...
| Acceptance | `make e2e-vendor` writes `go-test.json`, `go-test.log`, `containers.json`, `containers.log`, `environment.json`, `summary.md`, `vendor-profiles.json`, `vendor-images.json`, and `skip-summary.json`; missing licensed/vendor images cause documented skips, not false failures; Podman and containerlab runtimes are explicit. |
| Status | Implemented. |
...
|---|---|
| Runtime | GitHub-hosted Ubuntu runner with Podman and `podman-compose`. |
| Podman API | `/run/podman/podman.sock` is required by the dev container contract. |
| Upload condition | Artifacts are uploaded with `if: always()`. |
| Retention | 30 days. |
