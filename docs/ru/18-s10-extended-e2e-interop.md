...
| `testcontainers-go` | Отложить до isolated daemon-to-daemon S10.2 scenarios; не заменять existing compose/vendor topologies в S10.1. |
...
| Acceptance | Suite проходит из clean Podman state и пишет Go test JSON, daemon logs, CLI output и BFD pcapng artifacts. |
...
| Acceptance | Existing `make interop` и `make interop-bgp` evidence нормализована в один artifact directory без ослабления текущих targets. |
...
| Acceptance | Packet captures доказывают UDP 3784/3785 и overlay-port behavior; reserved `kernel`, `ovs`, `ovn`, `cilium`, `calico` и `nsx` backend names fail closed. |
