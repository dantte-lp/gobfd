...
| Linux Micro-BFD enforcement | Partial | GoBFD creates per-member RFC 7130 sessions, tracks aggregate state, has dry-run actuator wiring, and supports explicit Linux kernel-bond sysfs plus native OVSDB bonded-port enforcement with `owner_policy: allow-external`; NetworkManager D-Bus membership changes remain. |
...
| **S7.1** | Linux Micro-BFD actuator. | In progress: `MicroBFDActuator` hook, guarded `netio.LAGActuator` policy, config validation, daemon dry-run wiring, explicit kernel-bond backend, OVS CLI fallback, and native OVSDB backend are done; NetworkManager D-Bus backend remains. Commit: `feat(netio): add linux lag actuator`. |
...
| R6 | Micro-BFD is mistaken for universal Linux LAG enforcement. | M | H | Keep docs explicit: current code enforces explicit kernel-bond and native OVSDB paths with operator-approved external ownership; NetworkManager ownership needs S7.1f D-Bus backend. |
