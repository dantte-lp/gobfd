...
| Модель backend | `NewVXLANOverlayBackend` поддерживает `userspace-udp`; зарезервированные kernel/OVS/OVN/Cilium/NSX backend fail closed |
...
| Модель backend | `NewGeneveOverlayBackend` поддерживает `userspace-udp`; зарезервированные kernel/OVS/OVN/Cilium/NSX backend fail closed |
| Усиление receive path | Переиспользует ограниченные jumbo receive buffers; ожидаемые malformed/non-management пакеты отбрасываются на debug-уровне |
