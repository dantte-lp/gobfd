...
const (
	mergeArgumentCount = 4
	readArgumentCount  = 2
	writeArgumentCount = 3
)

var errUsage = errors.New(
	"invalid arguments: usage: artifactmerge " +
		"{merge output base.json bgp.json|write-image-id path id|read-image-id path}",
)
...
func run(args []string, stdout io.Writer) error {
	...
	case "merge":
		...
		if err := routingartifacts.Merge(args[1], []routingartifacts.Input{
			{Name: "interop", Path: args[2]},
			{Name: "interop-bgp", Path: args[3]},
		}); err != nil {
	...
	case "write-image-id":
		...
		if err := routingartifacts.WriteImageID(args[1], args[2]); err != nil {
	...
	case "read-image-id":
		...
		imageID, err := routingartifacts.ReadImageID(args[1])
		if err != nil {
