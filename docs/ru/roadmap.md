...
определяет разделение Final, таймаут и восстановление; письменный документ
ожидает согласования.
Сейчас Manager отклоняет изменения effective parameters; в его receipts ещё
нет `Updated`. Реализация `.1.1.1.4.2` зависит от согласования, затем следует
проверка с реальными пирами `.1.1.1.4.3`. Эти этапы ещё не означают готовность
обновления таймеров через reload.
