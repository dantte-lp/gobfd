...
engine, а operator-facing surface: dedicated API/CLI create flows, release
dry-run, pkg.go.dev polish и interop evidence перед `v0.5.0`.
...
| Linux VXLAN/Geneve dataplane coexistence | `userspace-udp` backend bind-ит `localAddr:4789` и `localAddr:6081`; reserved kernel/OVS/OVN/Cilium/NSX backend names fail closed. | RFC/config docs описывают explicit backend ownership и future non-userspace integration scope. | Partial |
...
| pkg.go.dev command page | `cmd/gobfd` требует расширенного package comment. | Plan помечает pkg.go.dev как S8 gap. | Open |
...
Command package comment должен кратко описывать daemon purpose, Linux
requirements, config path, capabilities, metrics, API endpoint и related
binaries. Это остаётся release-readiness задачей S8.
...
| S5 | Синхронизировать public control plane с daemon capabilities. | API vocabulary, session snapshots и CLI output покрывают Echo, Micro-BFD, VXLAN, Geneve; dedicated create flows remain. | `feat(api): expose advanced session type vocabulary` |
...
| S8 | `v0.5.0` release readiness without v1 bump. | Canonical docs layout, pkg.go.dev polish, release dry-run, changelog, SemVer tag plan. | `docs: sync release documentation layout` |
...
| Release/presentation quality | 76% | Changelog, standards and docs layout are aligned; pkg.go.dev and release dry-run remain. |
