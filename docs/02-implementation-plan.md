...
| **S7** | Independent production integration assets. | Generic Kubernetes manifests, Prometheus rules, FRR/GoBGP examples, optional public vendor examples, and failure drills are documented without referencing private repositories or site-specific topology. Commit: `feat(examples): add production integration assets`. |
