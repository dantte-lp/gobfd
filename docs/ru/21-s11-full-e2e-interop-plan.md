...
| Fix | Core E2E получает live service container ID через `test/internal/podmanapi` по Podman/Docker compose labels или deterministic compose names, затем читает logs через тот же Podman REST API socket. |
