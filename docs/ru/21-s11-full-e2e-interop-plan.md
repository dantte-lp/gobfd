...
| Fix | Core E2E получает live service container ID по labels `io.podman.compose.project` и `io.podman.compose.service`, затем читает logs через `test/internal/podmanapi` из Podman REST API socket. |
