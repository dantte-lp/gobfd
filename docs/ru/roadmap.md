...
попытками. v0.6.4 — первая опубликованная версия из `release/v0.6`; три ранее
зарегистрированных P1-находки ревью остаются в Beads и должны быть исправлены
через отдельно согласованное fix-forward решение до приёмки maintenance
baseline. Защищённая линия `release/v0.6` сохраняет GoBGP v3.37.0; разработка
...
поведения протокола BFD. Тег, GitHub Release, assets и OCI-образы v0.6.4
проверены, но baseline остаётся **в работе** до исправления и независимой
проверки P1-находок `gobfd-qj0.8.1.8.8`, `.8.9` и `.8.10`.
...
Release-задача `gobfd-qj0.8.1.15` завершена: неизменяемый `v0.6.4` указывает на
`b1c0bcd7d2e9abed00368b2082e34f521084c087`, все 12 assets и OCI indexes
проверены, а точная история fast-forward перенесена в `dev` без запуска GitHub
Actions. Qualification `gobfd-qj0.8.1.7` и независимая проверка
`gobfd-qj0.8.1.8` остаются открыты на трёх P1-находках выше.
...
v0.6 после исправления forward-only находок, оставшихся после v0.6.4**.
