...
|---|---|---|---|---|---|---|---|---|
| `make e2e-core` | `e2e-core` | Core E2E stack | Host Go 1.27 testcontainers-go with the Podman provider | `172.30.10.0/24` | GoBFD A/B, static simple-password auth, gobfdctl, metrics, reload, tshark | Go test JSON/log, bounded diagnostics, container logs/state, metrics checks, pcapng, packet CSV/TSV, summary | Exact test-owned container, network, and image cleanup | Authoritative Go runner; the retained `make e2e-core-testcontainers` compatibility target is the underlying gate. |
...
| `make e2e-routing` | `e2e-routing` | Go `e2ectl` routing aggregate | Podman Compose plus dev container Go test | `172.20.0.0/24`, `172.21.0.0/24` | `test/interop`, `test/interop-bgp`, tshark | Aggregate Go test JSON/log, container logs/state, merged pcapng, packet CSV, suite artifacts | `interopproject.Controller` holds one flock per exact Compose project, validates labels and fixed names, and removes exact resource snapshots; the merge container uses a unique non-Compose ownership label | Implemented for existing routing interop suites through the shared Go lifecycle API. |
| `make interop` | `e2e-routing` input | BFD interop stack | Podman Compose | `172.20.0.0/24` | GoBFD, FRR, BIRD3, Holo, Holo loader, Thoro/bfd, tshark, Go BFD invalid-vector generator | Go test output, pcap in capture container | Exact Compose project label preflight, `down --volumes --remove-orphans`, labelled fallback removal, and empty-label verification | Normalized by `make e2e-routing`. |
| `make interop-bgp` | `e2e-routing` input | BGP+BFD interop stack | Podman Compose | `172.21.0.0/24` | GoBFD, GoBGP, FRR, BIRD3, ExaBGP, tshark | Go test output, BGP/BFD state | Exact project resource snapshot cleanup through `interopproject.Controller` | Normalized by the Go-owned `make e2e-routing` lifecycle. |
...
| `make e2e-overlay` | `e2e-overlay` | Overlay boundary E2E | Prebuilt race test in a 2 CPU / 2 GiB Podman container | Isolated `--network none` loopback | VXLAN, Geneve Format A, reserved backend matrix | Go test JSON/log, container inspect/logs, synthetic codec CSV, summary | Exact created-container removal; image volumes ignored | Actual UDP tuple and lifecycle checks; external bounded capture remains required for wire qualification. |
...
| `make int-bgp-failover` | `e2e-routing` input | BGP fast-failover Go gate | Go 1.27 testcontainers-go with the Podman provider | `172.22.0.0/24` | GoBFD, GoBGP v3.37.0, FRR 10.7.1, tshark; shared trixie peers | Go test JSON/log, bounded diagnostics, container logs/state, pcapng, packet CSV, summary | Exact test-owned container, network, image-tag, and content-ID cleanup | Bounded builds/runtime; the operational Compose example uses the ownership-checked Go controller through `-up`, `-logs`, and `-down`. |
...
| E2E CI artifact workflow | Implemented in S10.7. |
| Podman lifecycle helper extraction | Implemented for routing through `interopproject.Controller` and `e2ectl`. |
| Host `go test` removal from full-cycle runners | Implemented for `e2e-routing`, `e2e-rfc`, `e2e-overlay`, `e2e-linux`, and `e2e-vendor`; `e2e-core` deliberately uses the pinned host/CI Go 1.27 toolchain to own Podman through testcontainers-go. |
