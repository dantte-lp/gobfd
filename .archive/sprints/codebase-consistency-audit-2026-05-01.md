...
| Linux VXLAN/Geneve dataplane coexistence | `userspace-udp` backend binds `localAddr:4789` and `localAddr:6081`; reserved kernel/OVS/OVN/Cilium/Calico/NSX backend names fail closed. | RFC/config docs describe explicit backend ownership and future non-userspace integration scope. | Partial |
...
| S7.2 | VXLAN/Geneve dataplane coexistence. | Done: `userspace-udp` is explicit, reserved kernel/OVS/OVN/Cilium/Calico/NSX backend names fail closed, and reconciliation reuses the runtime overlay backend. | `feat(netio): add overlay backend model` |
