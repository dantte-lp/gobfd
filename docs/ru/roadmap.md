...
v0.6 исправлены и приняты; финальная локальная квалификация baseline остаётся в
работе. Защищённая линия `release/v0.6` сохраняет GoBGP v3.37.0; разработка v1
и переход на GoBGP v4 продолжаются в `dev`.
...
Milestone Beads: `gobfd-qj0.8.1` — **открыт**.
...
остаётся **в работе** до приёмки финальной квалификации
`gobfd-qj0.8.1.7`.
...
отдельный forward-port. Qualification `gobfd-qj0.8.1.7` находится в работе;
независимая проверка
`gobfd-qj0.8.1.8` завершена после исправления и приёмки всех 13 дочерних
находок.
...
Milestone Beads: `gobfd-qj0.8.2` — **открыт; заблокирован до финальной приёмки
maintenance baseline v0.6**.
