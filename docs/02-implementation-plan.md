...
| Linux Micro-BFD enforcement | Partial | GoBFD creates per-member RFC 7130 sessions, tracks aggregate state, has dry-run actuator wiring, and supports explicit Linux kernel-bond sysfs plus transitional OVS CLI bonded-port enforcement with `owner_policy: allow-external`; native OVSDB and NetworkManager D-Bus membership changes remain. |
...
| **S7.1** | Linux Micro-BFD actuator. | In progress: `MicroBFDActuator` hook, guarded `netio.LAGActuator` policy, config validation, daemon dry-run wiring, explicit kernel-bond backend, and OVS CLI fallback are done; native OVSDB and NetworkManager D-Bus backends remain. Commit: `feat(netio): add linux lag actuator`. |
...
| **S7.1e** | Native OVSDB backend. | Add native OVSDB-backed LAG enforcement for OVS bonded ports while keeping `ovs-vsctl` as a fallback/diagnostics path. Commit: `feat(netio): add ovsdb lag backend`. |
| **S7.1f** | Optional NetworkManager backend. | Add NetworkManager D-Bus backend without making NetworkManager a required runtime dependency. Commit: `feat(netio): add networkmanager lag backend`. |
...
| R6 | Micro-BFD is mistaken for universal Linux LAG enforcement. | M | H | Keep docs explicit: current code enforces explicit kernel-bond and transitional OVS CLI paths with operator-approved external ownership; native OVSDB needs S7.1e and NetworkManager ownership needs S7.1f D-Bus backend. |
