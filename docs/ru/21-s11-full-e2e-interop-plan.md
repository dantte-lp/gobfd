...
| Fix | Core E2E получает live service container ID через `test/internal/podmanapi` по labels `io.podman.compose.project` и `io.podman.compose.service`, затем читает logs через тот же Podman REST API socket. |
