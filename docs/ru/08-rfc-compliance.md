...
| Daemon wiring | `micro_bfd.actuator` задаёт mode, backend, owner policy и действия с member |
| Kernel bond backend | `KernelBondLAGBackend` пишет `-member` / `+member` в Linux bonding sysfs |
| OVS backend | `OVSLAGBackend` запускает `ovs-vsctl del-bond-iface` / `add-bond-iface` |
...
member на существующем OVS bonded port через `ovs-vsctl`, но этот path является
transitional CLI fallback перед native OVSDB enforcement. Native OVSDB и
NetworkManager D-Bus backends остаются следующими implementation steps.
...
**Production-ограничение Linux**: `VXLANConn` владеет userspace UDP-сокетом на
`localAddr:4789`. Это подходит для лабораторного endpoint, выделенного
Management VNI endpoint или Linux VTEP, где GoBFD владеет сокетом. Если kernel
VXLAN, OVS, Cilium или другой dataplane уже владеет UDP 4789 в том же
namespace/local address, GoBFD может не привязать сокет или конкурировать с
dataplane. Для production Linux VTEP нужна явная модель socket ownership или
будущий pluggable backend, совместимый с kernel/OVS dataplane.
...
**Production-ограничение Linux**: `GeneveConn` владеет userspace UDP-сокетом на
`localAddr:6081`. Он валидирует RFC 9521 Format A packets, но не интегрируется
с socket ownership kernel Geneve, OVS/OVN или NSX dataplane. RFC 9521 также
наследует требование Geneve работать в traffic-managed controlled environment
или явно ограничивать BFD transmit rate, чтобы не получать ложные отказы из-за
перегрузки.
