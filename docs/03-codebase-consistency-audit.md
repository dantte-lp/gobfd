...
| Linux Micro-BFD enforcement | Per-member sessions, aggregate state tracking, actuator policy, dry-run config wiring, explicit kernel-bond sysfs backend, and transitional OVS CLI bonded-port backend exist; native OVSDB and NetworkManager backends do not exist yet. | RFC/config docs separate detect/report, dry-run policy, kernel-bond/OVS enforcement with `allow-external`, OVSDB research, and remaining NetworkManager D-Bus backend. | Partial |
...
Next sprint: S7.1e, `feat(netio): add ovsdb lag backend`.
...
an existing OVS bonded port through `ovs-vsctl`, but that path is a transitional
CLI fallback because Open vSwitch exposes native OVSDB JSON-RPC management. The
next implementation step is a native OVSDB backend before the separate
NetworkManager-owned load-balancing table gap.
...
Next sprints:
- S7.1e, `feat(netio): add ovsdb lag backend`
- S7.1f, `feat(netio): add networkmanager lag backend`
...
| S7 | Independent production integration readiness. | In progress: generic runbooks, Kubernetes manifest hardening, alert rule correction, FRR/GoBGP example documentation, public EOS verification notes, Micro-BFD actuator policy/config wiring, kernel-bond backend, OVS CLI fallback, and OVSDB research are done; remaining work is S7.1e/S7.1f/S7.2 implementation. | `feat(examples): add production integration assets` |
...
| S7.1 | Linux Micro-BFD enforcement. | In progress: Manager actuator hook, guarded `netio.LAGActuator` policy, config validation, daemon dry-run wiring, explicit kernel-bond backend, and OVS CLI fallback are done; native OVSDB and NetworkManager D-Bus backends remain. | `feat(netio): add linux lag actuator` |
...
| S7.1e | Native OVSDB backend. | OVSDB-backed LAG enforcement for OVS bonded ports while keeping `ovs-vsctl` as fallback/diagnostics. | `feat(netio): add ovsdb lag backend` |
| S7.1f | Optional NetworkManager backend. | NetworkManager D-Bus backend without making NetworkManager a required runtime dependency. | `feat(netio): add networkmanager lag backend` |
...
| Independent production applicability | 73% | Generic runbooks, BGP fast-failover examples, Micro-BFD dry-run wiring, kernel-bond enforcement, OVS CLI fallback, and OVSDB research are published; native OVSDB, NetworkManager, and overlay dataplane backends are still open. |
