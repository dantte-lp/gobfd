...
>
> **Блокер запуска (2026-09-08):** история закреплённого образа FRR `10.7.0`
> в registry подтверждает базу Alpine 3.22.5. Не запускайте приведённые ниже
> FRR-сценарии до квалификации замены в Beads `gobfd-qj0.8.2.8.5.2.4`.
> Имя vendor-образа не является исключением из запрета Alpine.
> [Официальный Debian-репозиторий](https://deb.frrouting.org/) сейчас предлагает
> FRR `10.7.1` для trixie: это кандидат на замену, не квалифицированный пир и
> не доказательство для `10.7.0`. Определения тестов сохранены, но квалификация
> обновления таймеров ещё не запускалась.
...
и переключение всех потребителей ещё не квалифицированы. Исторические
...
потребителей и live-квалификацию. Блокер запуска выше остаётся активным.
