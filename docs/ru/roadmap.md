...
![Следующий релиз](https://img.shields.io/badge/Next-v0.6.2-34a853?style=for-the-badge)
...
[`v0.6.1`](https://github.com/dantte-lp/gobfd/releases/tag/v0.6.1). Сначала
выпускается maintenance-релиз `v0.6.2`, затем additive production-контракт
`v1.0.0`. Линия v0.6.x будет поддерживаться в `release/v0.6` с GoBGP v3.37.0;
разработка v1 и переход на GoBGP v4 продолжаются в `dev`. Ветка
`release/v0.6` ещё не существует и будет создана только после включения её
набора правил защиты.
...
тестовую инфраструктуру без добавления нового поведения протокола BFD. v0.6.2
остаётся **в работе**, пока не проверены его тег и GitHub Release.
...
Milestone Beads: `gobfd-qj0.8.2` — **открыт; заблокирован до приёмки v0.6.2**.
